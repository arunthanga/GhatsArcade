...
export function WhatsAppButton({ phone, message, label = "Chat on WhatsApp" }: WhatsAppButtonProps) {
  const href = buildWhatsAppLink(phone, message);
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" data-testid="whatsapp-button">
      {label}
    </a>
  );
}

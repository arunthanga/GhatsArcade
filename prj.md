...
| SEO (sitemap, meta, schema.org) | In Progress | Root metadata + `sitemap.ts` done; `seo.ts` JSON-LD helpers are stubs (tests not yet written) |
| Data export (Owner-only) | Not Started | `csv.ts` + export route are stubs |
| Legal disclaimer component | In Progress | `LegalDisclaimer` rendered in root footer; tests not yet written |
